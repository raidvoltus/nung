import pandas as pd
import numpy as np
import os
import joblib
import logging
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler
from lightgbm import LGBMClassifier
from keras.models import Sequential
from keras.layers import Dense, LSTM, Dropout
from keras.callbacks import EarlyStopping

logging.basicConfig(level=logging.INFO)

# ========================= Fungsi Pelatihan Model ========================= #

def create_target(df):
    df['Target'] = (df['Close'].shift(-1) > df['Close']).astype(int)
    return df.dropna()

def load_features_targets(df):
    feature_cols = [
        'SMA_5', 'SMA_10', 'EMA_5', 'EMA_10', 'RSI_14', 'MACD', 'MACD_signal',
        'BB_Middle', 'BB_Upper', 'BB_Lower', 'Stochastic_K', 'Stochastic_D',
        'ATR_14', 'Volume', 'Volume_MA20', 'Tenkan_sen', 'Kijun_sen',
        'Senkou_Span_A', 'Senkou_Span_B', 'Chikou_Span', 'SAR',
        'Fibo_0.236', 'Fibo_0.382', 'Fibo_0.5', 'Fibo_0.618', 'Fibo_0.786'
    ]
    df = df.dropna(subset=feature_cols + ['Target'])
    X = df[feature_cols]
    y = df['Target']
    return X, y

def train_lightgbm(X_train, y_train, symbol):
    model = LGBMClassifier(n_estimators=100, learning_rate=0.05, max_depth=6)
    model.fit(X_train, y_train)
    os.makedirs("data/models", exist_ok=True)
    joblib.dump(model, f"data/models/{symbol}_lgb.pkl")
    logging.info(f"✅ LightGBM model {symbol} tersimpan.")
    return model

def train_lstm(X_train, y_train, symbol):
    X_train = np.reshape(X_train.values, (X_train.shape[0], 1, X_train.shape[1]))

    model = Sequential()
    model.add(LSTM(units=64, return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))
    model.add(Dropout(0.2))
    model.add(LSTM(units=32))
    model.add(Dropout(0.2))
    model.add(Dense(1, activation='sigmoid'))

    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

    early_stop = EarlyStopping(monitor='loss', patience=5, restore_best_weights=True)
    model.fit(X_train, y_train, epochs=20, batch_size=32, verbose=0, callbacks=[early_stop])

    os.makedirs("data/models", exist_ok=True)
    model.save(f"data/models/{symbol}_lstm.h5")
    logging.info(f"✅ LSTM model {symbol} tersimpan.")
    return model

# ========================= Fungsi Utama untuk Training ========================= #

def retrain_model(df, symbol):
    if df.empty:
        logging.warning(f"❌ Data untuk {symbol} kosong.")
        return

    df = create_target(df)
    X, y = load_features_targets(df)

    # Standardisasi
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    joblib.dump(scaler, f"data/models/{symbol}_scaler.pkl")

    X_train, _, y_train, _ = train_test_split(X_scaled, y, test_size=0.2, shuffle=False)

    train_lightgbm(pd.DataFrame(X_train, columns=X.columns), y_train, symbol)
    train_lstm(pd.DataFrame(X_train, columns=X.columns), y_train, symbol)

# ========================= Untuk Manual Run ========================= #

if __name__ == '__main__':
    if not os.path.exists('data/historical_data.csv'):
        logging.error("❌ File data/historical_data.csv tidak ditemukan.")
    else:
        df = pd.read_csv('data/historical_data.csv')
        retrain_model(df, symbol="ALL")
